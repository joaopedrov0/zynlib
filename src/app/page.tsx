...
function renderDisciplineCard(discipline: DisciplineRecord, canManage: boolean) {
  return (
    <div
      key={discipline.id}
    ...
    >
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="w-10 h-10 rounded-lg bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 flex items-center justify-center group-hover:scale-105 transition-transform">
      ...
      <Link
        ...
        className="mt-6 flex items-center text-xs font-medium text-indigo-600 dark:text-indigo-400 group-hover:translate-x-1 transition-transform"
      >
        ...
        {disciplines.length === 0 ? (
          <div className="text-center py-16 px-4 rounded-2xl border border-dashed border-zinc-300 dark:border-zinc-800 bg-white/50 dark:bg-zinc-900/30">
            ...
            <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400 max-w-md mx-auto">
              Adicione suas primeiras disciplinas pelo botão acima ou execute o seed do banco de dados.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {disciplines.map((d) => renderDisciplineCard(d, canManage))}
          </div>
        )}
      </main>
